package com.project.fashion.chat
...
class GeminiAdapter(var context: Context, var list: ArrayList<DataResponse>) :
    ...
    private inner class UserViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(position: Int) {
            ...
            text.text = data.prompt
            text.visibility = if (data.prompt.isNotBlank()) View.VISIBLE else View.GONE

            // Set image visibility and URI
            if (data.imageUri.isNotBlank() && data.imageUri != "imageTag") {
